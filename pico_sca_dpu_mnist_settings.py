#
# Modified by - Sudarshan Sharma
#         PhD Student, Green Lab, Gerogia Tech
...
'''
This file has the settings to capture channel B traces using channel A as trigger.
I am using a pulse width trigger as written in the example. I have modified some of the 
constants such that I can capture the traces correctly.
'''
...
def return_traces(plot_trace = 0):
    ...
    chBRange = 2 # This sets the range for Channel B to PS6000_40mmV
    status["setChB"] = ps.ps6000SetChannel(chandle, 1, 1, 0, chBRange, 0, 0)
    assert_pico_ok(status["setChB"])

    # Set up window pulse width trigger on A
    # for the other channels do not set any trigger state
    ...
    #This max ADC corresponds to the 0.5V, we are triggering at the falling edge and then capture traces pre-trigger
    ...
    #This needs to be set everytime such that we collect the appropriate traces
    preTriggerSamples = 39*1900
    postTriggerSamples = 39*300
    ...
    # segment index = 0
    timebase = 1800 # Higher the timebase lower the resolution
    ...
    status["setDataBuffersB"] = ps.ps6000SetDataBuffers(chandle, 1, ctypes.byref(bufferAMax), ctypes.byref(bufferAMin), maxSamples, 0)
